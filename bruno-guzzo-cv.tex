...
%	\vspace{5pt}
	...
	\begin{minipage}[t]{0.65\linewidth}
		...
		\jobsection{\textbf{Cloud Architect} (Mid-Senior Consultant)}{January 2025 -- Present}{\href{https://www.reply.com/go-reply/en}{Go Reply, Turin}}
		...
		\begin{itemize}[leftmargin=*, nosep]
			...
			% Azimut
			\vspace{6pt}
			\item \footnotesize Directed a cross-functional team of developers in delivering a \textbf{B2C cloud-native mobile application} enabling after-sales support, digital documentation access via \textbf{Gen-AI RAG chat-bot} and seamless multi-cloud \textbf{CRM integration}. 
			Engineered a resilient distributed \textbf{serverless micro-services architecture} coordinated using \textbf{Google Pub/Sub} to enforce \textbf{fault isolation} and ensure \textbf{high availability} under unpredictable load.
			
			
%			Guided the adoption of a distributed serverless \textbf{micro-services} architecture coordinated using \textbf{Google Pub/Sub} to enforce \textbf{fault isolation} and scalability. 
		...
		\begin{itemize}[leftmargin=*, nosep]
			...
			Led the platform migration to Google Cloud, implementing \textbf{Infrastructure as Code} (IaC) best practices using a full \textbf{Terraform foundation} and \textbf{GCP Fabric modules} for custom service configuration and resources tracking.
			This robust IaC approach delivered a \textbf{30\% reduction in infrastructural fixed-costs}, enhancing overall system efficiency.
...
%		\vspace{4pt}
		...
		\begin{itemize}[leftmargin=*, nosep, itemsep=2pt]
			...
			\item \textbf{Infra}: VPC, Load Balancer, IAM, Cloud Armor, Cloud DNS, Cloud CDN, Cloud Task, Cloud Pub/Sub, Apigee proxy, Cloud Logging, Cloud Monitoring, Cloud Trace
		\end{itemize}
		\vspace{5pt}
		...
		\begin{itemize}[leftmargin=*, nosep, itemsep=2pt]
			\footnotesize
			\item \textbf{Frameworks}: PyTorch, Langchain, Hugging Face Transformers
			\item \textbf{Libraries}: Pandas, NumPy, Scikit-learn
			\item \textbf{Concepts}: LLM, RAG, NLP, Fine-Tuning, Vector Databases, Embeddings, Anomaly Detection
		\end{itemize}
		\vspace{5pt}
		...
		\begin{itemize}[leftmargin=*, nosep, itemsep=2pt]
			...
			\item \textbf{CI/CD}: Cloud Build, GitLab CI, GitHub Actions, Artifact Registry, Jenkins, SonarQube
			\item \textbf{Containers}: Docker, Docker Compose
			\item \textbf{Others}: Linux, Git, Bash scripting, NginX, Shell Scripting, Monitoring, Logging, Tracing
		\end{itemize}
		\vspace{5pt}
		...
		\begin{itemize}[leftmargin=*, nosep, itemsep=2pt]
			...
			\item \textbf{Others}: Jira, Sprint Planning, Cloud-Native, Design Patterns, Keycloak
		\end{itemize}
		\vspace{7pt}
